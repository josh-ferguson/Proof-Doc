...
async function showContent() {
  let res = await fetch('/findWork/displayAllWork');
  ...
  item = temp.content.querySelector(".findWorkUploaded");
  ...
  for (i = 0; i < workArr.length; i++) {
    ...
    workLink.href = "/findShowWork?workID=" + workArr[i].workID;
...
async function showAccount() {
  let res = await fetch('/findWork/getAcountName');
